...
import (
	...
	"gitlab-nomo.credissimo.net/nomo/cosmzone/nolus/x/mint/types"
)
...
var (
	...
	expectedTokensInFormula = []int64{
		2005393779413, 1960065211135, 1915701583592, 1872292817451, 1829821427258,
		1788274927426, 1747641763937, 1707904450838, 1669053479559, 1631072341866, 1593949550419, 1557669553370,
		1522218863850, 1487586973442, 1453755394621, 1420715576293, 1388449010421, 1356946272040, 1326192726904,
		1296171971295,
		1266873433466, 1238282626336, 1210387040993, 1183170189298, 1156621501589, 1130725550707, 1105469766714,
		1080839681981,
		1056822747547, 1033404455241, 1010572355682, 988309900621, 966607542783, 945449851062, 924822259131,
		904712277524,
		...
		628832247789, 615711287449, 602849274537, 590231718334, 577847074639, 565680852982, 553718544290, 541947658048,
		530354632472,
		518925029729, 507645305912, 496503917208, 485484425469, 474574269364, 463760957802, 453029982379, 442366834512,
		431760023172, 421194004353, 410656287539, 400132381360, 389609724801, 379073879265, 368512283906, 357910464737,
		347253860867,
...
func Test_CalcTokensDuringFormula_WhenUsingConstantIncrements_OutputsPredeterminedAmount(t *testing.T) {
	...
	for i := int64(0); i < secondsInFormula; i++ {
		...
		if i%secondsInMonth == 0 {
			fmt.Printf(
				"%v Month, %v Minted, %v Total Minted(in store), %v Returned Total, %v Norm Time, %v Recieved in this block \n",
	...
	fmt.Printf(
		"%v Returned Total, %v Total Minted(in store), %v Norm Time \n",
	...
	if !expectedCoins60Sec.Equal(mintedCoins) || !expectedCoins60Sec.Equal(minter.TotalMinted) {
		t.Errorf(
			"Minted unexpected amount of tokens, expected %v returned and in store, actual minted %v, actual in store %v",
			expectedCoins60Sec, mintedCoins, minter.TotalMinted,
		)
	}
	if !expectedNormTime20Sec.Equal(minter.NormTimePassed) {
		t.Errorf(
			"Received unexpected normalized time, expected %v, actual %v", expectedNormTime20Sec, minter.NormTimePassed,
		)
	}
}
...
func Test_CalcTokensDuringFormula_WhenUsingVaryingIncrements_OutputExpectedTokensWithinEpsilon(t *testing.T) {
	...
	for i := int64(0); timeOffset < nanoSecondsInPeriod; {
		...
		if (timeOffset-prevOffset)/nanoSecondsInMonth.TruncateInt64() != (timeOffset+i-prevOffset)/nanoSecondsInMonth.TruncateInt64() {
			...
			fmt.Printf(
				"%v Month, %v Minted, %v Total Minted(in store), %v Returned Total, %v Norm Time, %v Received in this block \n",
			...
			if mintedMonth.Sub(sdk.NewInt(expectedTokensInFormula[month-1])).Abs().GT(monthThreshold) {
				t.Errorf(
					"Minted unexpected amount of tokens for month %d, expected [%v +/- 100*10^6], actual %v",
	...
	fmt.Printf(
		"%v Returned Total, %v Total Minted(in store), %v Norm Time \n", mintedCoins, minter.TotalMinted,
		minter.NormTimePassed,
	)
	if expectedCoins60Sec.Sub(mintedCoins).Abs().GT(mintThreshold) || expectedCoins60Sec.Sub(minter.TotalMinted).Abs().GT(mintThreshold) {
		t.Errorf(
			"Minted unexpected amount of tokens, expected [%v +/- 10*10^6] returned and in store, actual minted %v, actual in store %v",
			expectedCoins60Sec, mintedCoins, minter.TotalMinted,
		)
	}
	if expectedNormTime20Sec.Sub(minter.NormTimePassed).Abs().GT(normTimeThreshold) {
		t.Errorf(
			"Received unexpected normalized time, expected [%v +/- 0.000001], actual %v",
...
func Test_CalcTokensFixed_WhenNotHittingMintCapInAMonth_OutputsExpectedTokensWithinEpsilon(t *testing.T) {
	...
	if types.FixedMintedAmount.Sub(mintedCoins).Abs().GT(mintThreshold) || types.FixedMintedAmount.Sub(minter.TotalMinted).Abs().GT(mintThreshold) {
		t.Errorf(
			"Minted unexpected amount of tokens, expected [%v +/- 10^6] returned and in store, actual minted %v, actual in store %v",
	...
	if (types.MonthsInFormula.Add(sdk.OneDec())).Sub(minter.NormTimePassed).Abs().GT(normTimeThreshold) {
		t.Errorf(
			"Received unexpected normalized time, expected [%v +/- 0.000001], actual %v", expectedNormTime20Sec,
			minter.NormTimePassed,
		)
	}
}
...
func Test_CalcTokensFixed_WhenHittingMintCapInAMonth_DoesNotExceedMaxMintingCap(t *testing.T) {
	...
	if types.MintingCap.Sub(minter.TotalMinted).Abs().GT(sdk.ZeroInt()) {
		t.Errorf(
			"Minting Cap exeeded, minted total %v, with minting cap %v",
			minter.TotalMinted, types.MintingCap,
		)
	}
	if halfFixedAmount.Sub(mintedCoins).Abs().GT(mintThreshold) {
		t.Errorf(
			"Minted unexpected amount of tokens, expected [%v +/- 10^6] returned and in store, actual minted %v",
...
func Test_CalcTokens_WhenMintingAllTokens_OutputsExactExpectedTokens(t *testing.T) {
	...
	for i := int64(0); timeOffset < offsetNanoInPeriod; {
		...
		if (timeOffset-prevOffset)/nanoSecondsInMonth.TruncateInt64() != (timeOffset+i-prevOffset)/nanoSecondsInMonth.TruncateInt64() {
			...
			fmt.Printf(
				"%v Month, %v Minted, %v Total Minted(in store), %v Returned Total, %v Norm Time, %v Received in this block \n",
				(timeOffset+i)/nanoSecondsInMonth.TruncateInt64()-monthsSoFar, mintedMonth, minter.TotalMinted,
				mintedCoins, minter.NormTimePassed, coins,
			)
			prevOffset = timeOffset
			mintedMonth = sdk.ZeroInt()
		}
...
func Test_CalcIncrementDuringFormula_OutputsExpectedIncrementWithinEpsilon(t *testing.T) {
	...
	if sumIncrements5s.Sub(types.AbsMonthsRange).Abs().GT(normTimeThreshold) {
		t.Errorf(
			"Increment with 5 second step results in range %v, deviating with more than epsilon from expected %v",
			sumIncrements5s, types.AbsMonthsRange,
		)
	}
	if sumIncrements30s.Sub(types.AbsMonthsRange).Abs().GT(normTimeThreshold) {
		t.Errorf(
			"Increment with 30 second step results in range %v, deviating with more than epsilon from expected %v",
			sumIncrements30s, types.AbsMonthsRange,
		)
	}
	if sumIncrements60s.Sub(types.AbsMonthsRange).Abs().GT(normTimeThreshold) {
		t.Errorf(
			"Increment with 60 second step results in range %v, deviating with more than epsilon from expected %v",
